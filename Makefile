...
docker-build: ## Build a container image and add the version and latest tag
	@if command -v docker > /dev/null; then \
		echo "Building..."; \
		docker buildx build -t jsfelipearaujo/appointment-service:latest -t jsfelipearaujo/appointment-service:$$(git rev-parse --short HEAD) .; \
	else \
...
docker-push: ## Push the container image to the registry
	...
	docker push jsfelipearaujo/appointment-service:$$(git rev-parse --short HEAD)
